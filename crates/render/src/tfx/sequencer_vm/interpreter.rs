...
use crate::{Renderer, tfx::expression_vm::helpers, util::math::Vec4Ext};
    ...
    pub fn evaluate(&mut self, constants: &[Vec4], out: &mut [Vec4]) -> anyhow::Result<()> {
        ...
        'exec: while self.ip < self.data.len() {
            ...
            match op {
                ...
                Opcode::Unknown0x24 => {
                    set_top!(helpers::bytecode_op_24(cached_top));
                }
                Opcode::Unknown0x25 => {
                    set_top!(helpers::bytecode_op_25(cached_top));
                }
                Opcode::Triangle => {
                    set_top!(helpers::bytecode_op_triangle(cached_top));
                }
                Opcode::Jitter => {
                    set_top!(helpers::bytecode_op_jitter(cached_top));
                }
                Opcode::Wander => {
                    set_top!(helpers::bytecode_op_wander(cached_top));
                }
                Opcode::Rand => {
                    set_top!(helpers::bytecode_op_rand(cached_top));
                }
                ...
                Opcode::VectorRotationsSin => {
                    set_top!(helpers::_trig_helper_vector_sin_rotations_estimate(
                        cached_top
                    ));
                }
                Opcode::VectorRotationsCos => {
                    set_top!(helpers::_trig_helper_vector_cos_rotations_estimate(
                        cached_top
                    ));
                }
                Opcode::VectorRotationsSinCos => {
                    set_top!(helpers::_trig_helper_vector_sin_cos_rotations_estimate(
                        cached_top
                    ));
                }
                ...
                Opcode::Spline4Const => {
                    ...
                    cached_top = helpers::bytecode_op_spline4_const(
                ...
                Opcode::Spline8Const => {
                    ...
                    cached_top = helpers::bytecode_op_spline8_const(
                ...
                Opcode::Spline8ChainConst => {
                    ...
                    let cl = &constants[constant_start as usize..constant_start as usize + 10];
                    cached_top = helpers::bytecode_op_spline8_chain_const(
                ...
                Opcode::Gradient4Const => {
                    ...
                    cached_top = helpers::bytecode_op_gradient4_const(
                ...
                Opcode::Unknown0x49 => {
                    ...
                    set_top!(helpers::bytecode_op_unk3b_const(cached_top, cl));
                }
                Opcode::Unknown0x4A => {
                    let channel = ptr[1];
                    let val =
