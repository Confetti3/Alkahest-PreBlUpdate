...
fn main() -> Result<()> {
    let packages = std::env::args()
        .nth(1)
        .context("usage: shadowkeep_pass_probe <packages-dir>")?;
    ...
    for name in [
        ...
        println!(
            "{name} tag={tag} bind={:?} states=0x{:08x} scopes=0x{:08x}",
            technique.bind_mode, technique.states.0, technique.used_scopes.0
        );
        for (label, shader) in [
            ("vs", &technique.shader_vertex),
            ...
            println!(
                ...
                shader.constants.constant_buffer_slot,
                shader.constants.constant_buffer,
                shader.constants.bytecode.len(),
                shader.constants.bytecode_constants.len(),
                shader.constants.inline_constants.len(),
                shader.constants.samplers.len(),
                shader.textures.len()
            );
            println!("    bytecode={:02x?}", shader.constants.bytecode);
            println!(
                "    inline_constants[0..8]={:?}",
                &shader.constants.inline_constants
                    [..shader.constants.inline_constants.len().min(8)]
            );
            println!(
                "    textures={:?}",
                shader
