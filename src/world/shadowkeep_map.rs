    ...
    for table_hash in table_hashes {
            ...
            let table: SShadowkeepMapDataTable = match package_manager().read_tag_struct(table_hash) {
                ...
                Err(error) => {
                    ...
                    report.diagnostic(progress, MapLoadDiagnostic {
                        table: table_hash, entry_offset: 0, resource_class: 0,
            ...
            for entry in &table.data_entries {
                ...
                let transform = Transform::new(
                    entry.translation.xyz(), entry.rotation, Vec3::splat(entry.translation.w),
                );
                if !entry.entity.is_none() {
                    report.entity_entries += 1;
                    if let Ok(entity) = package_manager().read_tag_struct::<SShadowkeepEntity>(entry.entity) {
                        for resource_ref in &entity.entity_resources {
                            ...
                            if resource.resource.resource_type != 0x8080_72B8 || !resource.definition.is_valid {
                                continue;
                            }
                            report.rigid_entities += 1;
                            let result = (|| -> anyhow::Result<()> {
                                let bytes = package_manager().read_tag(resource_ref.resource.taghash())?;
                                let mut cursor = Cursor::new(bytes);
                                cursor.seek(SeekFrom::Start(resource.definition.offset))?;
                                let component = alkahest_data::shadowkeep::SShadowkeepRigidModelComponent::read_ds(&mut cursor)?;
                                ...
                                progress.visual_resources_loaded.fetch_add(1, Ordering::Relaxed);
                            ...
                            if let Err(error) = result {
                                report.skipped_resources += 1;
                                report.diagnostic(progress, MapLoadDiagnostic {
                                    table: table_hash, entry_offset: entry.data_resource.offset,
                ...
                match entry.data_resource.resource_type {
                        ...
                        let result = (|| -> anyhow::Result<()> {
                            ...
                            for (index, light) in collection.lights.iter().enumerate() {
                                ...
                                let transform = collection
                                    ...
                                    .get(index)
                                    .copied()
                                    .unwrap_or(alkahest_data::shadowkeep::SShadowkeepRotationTranslation {
                    ...
                    SHADOWING_LIGHT => {
                        ...
                        let result = (|| -> anyhow::Result<()> {
                            ...
                            let light: SShadowkeepShadowingLight = package_manager().read_tag_struct(light_tag)?;
                            ...
                            world.spawn((
                                Transform::new(entry.translation.xyz(), entry.rotation, Vec3::splat(entry.translation.w)),
                                DynamicRenderObject::new(renderer.add_object(RenderObject::new(
                                    TfxFeatureRenderer::DeferredLights,
                                    renderer_object,
                                ))),
                    ...
                    STATIC_PLACEMENT => {
                        ...
                        let result = (|| -> anyhow::Result<()> {
                            ...
                            let placement: SShadowkeepStaticPlacement = package_manager().read_tag_struct(header_tag)?;
                            ...
                            progress.gpu_assets_requested.fetch_add(1, Ordering::Relaxed);
                            world.spawn((StaticRenderObject::new(renderer.add_object(RenderObject::new(
                        ...
                        if let Err(error) = result {
                            ...
                            report.diagnostic(progress, MapLoadDiagnostic {
                                table: table_hash, entry_offset: entry.data_resource.offset,
                                resource_class: STATIC_PLACEMENT, error: format!("static placement: {error:#}"),
                            });
                        }
                    }
                    TERRAIN_PLACEMENT => {
                        ...
                        if let Err(error) = result {
                            ...
                            report.diagnostic(progress, MapLoadDiagnostic {
                                table: table_hash, entry_offset: entry.data_resource.offset,
                                resource_class: TERRAIN_PLACEMENT, error: format!("terrain placement: {error:#}"),
                            });
                        }
                    }
                    _ => {}
